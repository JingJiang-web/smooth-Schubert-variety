...
w_input = [int(i) for i in w_input.strip('[]').split(',') if i.strip()]
...
for t in ["A3", "D4"]:
    ...
    for i, item in enumerate(subs[t][:2000], start=1):
        ...
        gs = gammas_from_word(w1, Cmat)
        inv_w = list(reversed(w1))
        gamma_set = {tuple(g) for g in gs}
        pos_roots = item["pos_roots"]
        sub_pos_set = {tuple(r) for r in pos_roots}
        positive_roots_set = {tuple(r) for r in positive_roots}
        sub_positive_set = sub_pos_set & positive_roots_set
        ...
        def build_I_sigma_and_reflection_product(
            ...
            def beta_to_ambient(beta_coeffs):
                n = len(betas[0])
                out = [0] * n
                for c, b in zip(beta_coeffs, betas):
                    for j in range(n):
                        out[j] += int(c) * int(b[j])
                return tuple(out)

            def ambient_to_beta(root):
                return tuple(int(c) for c in solve_in_beta_basis(betas, root))

            def apply_simple_reflection_in_beta_coords(coeffs, i):
                coeffs = list(coeffs)
                pairing = sum(coeffs[j] * A_sub[j][i] for j in range(len(coeffs)))
                coeffs[i] -= pairing
                return tuple(coeffs)

            def reflect_ambient_root_by_beta_simple(root, i):

                coeffs = ambient_to_beta(root)
                new_coeffs = apply_simple_reflection_in_beta_coords(coeffs, i)
                return beta_to_ambient(new_coeffs)

            def enumerate_positive_roots_in_subsystem_from_betas():
                ...
                while head < len(queue):
                    r = queue[head]
                    head += 1
                    coeffs = ambient_to_beta(r)

                    for i in range(k):
                        pairing = sum(coeffs[j] * A_sub[j][i] for j in range(k))
                        if pairing < 0:
                            new_coeffs = list(coeffs)
                            new_coeffs[i] += 1
                            new_root = beta_to_ambient(new_coeffs)
                            if new_root not in pos:
                                pos.add(new_root)
                                queue.append(new_root)

                return pos

            positive_root_set = enumerate_positive_roots_in_subsystem_from_betas()

            def choose_right_descent(I_set):
                simple_roots = [root_tuple(b) for b in betas]

                candidates = []
                for i, bi in enumerate(simple_roots):
                    if bi not in I_set:
                        continue
                    ...
                    for g in I_set:
                        if g == bi:
                            continue
                        sg = reflect_ambient_root_by_beta_simple(g, i)
                        if sg not in positive_root_set:
                            ok = False
                            break
                ...
                if not candidates:
                    raise ValueError(
                        f"Cannot peel a simple root from inversion set: {sorted(I_set)}"
                    )

                return candidates[0]

            def peel_once(I_set, i):
                bi = root_tuple(betas[i])
                new_set = set()
                for g in I_set:
                    if g == bi:
                        continue
            ...
            def sigma_from_inversion_set():
                I_set = {root_tuple(r) for r in I_sigma_roots}
                sigma_right = []

                while I_set:
                    i = choose_right_descent(I_set)
                    sigma_right.append(i + 1)
                    I_set = peel_once(I_set, i)

                return list(reversed(sigma_right))

            sigma = sigma_from_inversion_set()
...
print("=" * 90)
print("X(w_0w) is non-smooth" if any_red else "X(w_0w) is smooth")